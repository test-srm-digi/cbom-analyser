  ...
  const pqcStep = pqcThresholdEnabled ? `
      - name: Check PQC Readiness Threshold
        run: |
          ...
          # Parse CBOM and check quantum-safe percentage
...
function getScannerStep(language: string): string {
  switch (language) {
    ...
    default:
      return `
      - name: Run generic crypto scanner
        run: |
